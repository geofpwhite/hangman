...
func game(inputChannel chan inputInfo, timeoutChannel chan int, outputChannel chan clientState, newGameChannel chan bool) {
	inputChannels := []chan inputInfo{}
	tickerTimeoutChannel := make(chan (int))
	tickerInputChannel := make(chan (inputInfo))
	inputChannels = append(inputChannels, tickerInputChannel)
	gState := gStates[0]
	go gState.runTicker(tickerTimeoutChannel, tickerInputChannel)
	for {
		select {
		...
		case <-newGameChannel:
			...
			inputChannels = append(inputChannels, newTickerInputChannel)
		...
		case info := <-inputChannel:
			...
			fmt.Println(info)
			fmt.Println(gState)
			inputChannels[info.GameIndex] <- info //for the ticker to handle
			//main part of loop
